...
<head>
  ...
  <title>Calendrier de l'Avent – Devine le/la collègue</title>
  <style>
    :root{
      --bg:#0c111d;          /* bleu nuit profond */
      --card:#151c2f;        /* carte sombre */
      --muted:#7f8aa3;       /* gris bleuté */
      --accent:#e0b567;      /* doré */
      --accent-2:#24b47e;    /* vert succès */
...
<body>
  <div class="wrap">
    <div class="header">
      <div>
        <div class="title">Calendrier de l’Avent • Devine le/la collègue</div>
        <div class="subtitle">Chaque jour de décembre, ouvre la case, lis l’anecdote et propose le nom du/de la collaborateur·rice ✨</div>
      </div>
      <div class="toolbar">
    ...
    <div class="chips" style="margin-top:14px">
      ...
      <span class="chip">Intégrable dans Notion (Embed)</span>
    </div>
  </div>
  ...
  <div class="modal-backdrop" id="backdrop" aria-hidden="true">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
      ...
      <div class="modal-body">
        ...
        <div class="form">
          <label class="hint">Ta proposition (devine à qui appartient l’anecdote) :</label>
          <div class="row">
            <select id="answerSelect"></select>
            <button class="btn" id="submit">Valider</button>
          </div>

          <label class="hint">Un message sympa (optionnel) à laisser à la personne :</label>
          <textarea id="note" placeholder="Ex. : Trop drôle ! Je parie que c’est toi 😄"></textarea>
          <div class="hint">Les réponses restent sur <b>ton appareil</b> (localStorage). Rien n’est envoyé sur un serveur.</div>
        ...
        <div class="footer">
          ...
          <button class="btn" id="revealBtn">Révéler la réponse</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
     ...
     **********************/
    const TEAM = [
      "Carine", "Tom", "Annabelle", "Amélie", "Hugo", "Léa", "Jules", "Camille", "Louis", "Sarah",
      "Nina", "Paul", "Emma", "Théo", "Inès", "Maxime", "Manon", "Chloé", "Axel", "Noémie",
      "Romain", "Lucie", "Yasmine", "Victor"
    ];

    // 24 anecdotes – exemple; mets les tiennes. "answer" = prénom exact affiché dans TEAM.
    const DAYS = Array.from({length:24}, (_,i)=>i+1).map((day)=>({
      day,
      anecdote: `Anecdote du jour ${day} — remplace ce texte par une vraie anecdote croustillante.`,
      answer: TEAM[day-1] || TEAM[0],
      // Réponses acceptées en plus (variantes, surnoms)
      aliases: TEAM[day-1] ? [TEAM[day-1], TEAM[day-1].toLowerCase()] : []
    }));
    ...
    function isUnlocked(day){
      // Débloque en décembre jusqu'au jour courant, sauf en mode dev (tout débloqué)
    ...
    function buildOptions(){
      ...
      const none = el("option", {value:"", text:"— Sélectionne un nom —"});
    ...
    function openModal(day){
      ...
      revealBox.textContent = `Réponse : ${d.answer}`;
    ...
    submitBtn.addEventListener("click", ()=>{
      ...
      if(good){
        ...
        result.textContent = "Mmmh, pas certain·e… essaie encore 😄";
    ...
    revealBtn.addEventListener("click", ()=>{
      ...
      result.textContent = `C'était ${d.answer} ✨`;
      result.className = "result ok";
    });
    ...
    $("reset").addEventListener("click", ()=>{
      if(confirm("Réinitialiser tes réponses et le score ?")){
